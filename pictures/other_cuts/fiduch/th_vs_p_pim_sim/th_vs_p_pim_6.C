...
TH1 *h2;
c->cd();
MyFile->GetObject("th_vs_p_sim/th_vs_p_pim_1_sim_6",h);
h->Draw("colz");
...
TF1 *f5 = new TF1("f3",funct2, 0.05,0.5,2);
 f5->SetParameter(0,70.);
 f5->SetParameter(1,-0.15);
  f5->SetLineColor(kBlack);
  f5->SetLineWidth(2);
// f5->Draw("same"); 
 
TF1 *f6 = new TF1("f6",funct2, 0.05,0.7,2);
 f6->SetParameter(0,50.);
...
TF1 *f7 = new TF1("f6",funct2, 0.05,0.7,2);
 f7->SetParameter(0,55.);
 f7->SetParameter(1,-0.15);
  f7->SetLineColor(kBlack);
  f7->SetLineWidth(2);
// f7->Draw("same");
      

c->SaveAs("pim_th_vs_p_sim_sector6.pdf");   
};
