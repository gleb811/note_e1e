...
TH1 *h2;
c->cd();
MyFile->GetObject("th_vs_p_sim/th_vs_p_pim_1_sim_2",h);
...
TF1 *f4 = new TF1("f2",funct2, 0.05,2.,1);
 f4->SetParameter(0,2.);
...
TF1 *f5 = new TF1("f3",funct2, 0.05,2.,1);
 f5->SetParameter(0,-2.5);
 //f5->SetParameter(1,0);
  f5->SetLineColor(kBlack);
  f5->SetLineWidth(2);
f5->Draw("same");   

c->SaveAs("pim_th_vs_p_sim_sector2.pdf");    
};
...
double funct2(double *x, double *par) {
   double s;
     ...
     s=36.152+3.69909e-05/(5.40783e-06*x[0]+1.81169e-07)+par[0];
