...
TH1 *m_pip_p,*m_pip_pim,*m_pim_p;



TH1 *m_pip_p_corr,*m_pip_pim_corr,*m_pim_p_corr;

//TH1D *m_pip_p_corr,*m_pip_pim_corr,*m_pim_p_corr;
...
TFile *file_all_w = new TFile("empty_cells_mass_corr_all_w.root","READ");
TFile *file_cntr_w = new TFile("empty_cells_mass_corr_cntr_w.root","READ");
TFile *file_model = new TFile("../out_cr_sec_new.root","READ");
...
read_data_all_w(file_all_w,i,qq2);
read_data_cntr_w(file_cntr_w,i,qq2);
//read_model(file_model,i,qq2);

//draw_1d_canvas(i,qq2);

c->cd(1);
c->cd(1)->SetLeftMargin(0.2);
m_pip_p->Scale(1./(m_pip_p->GetEntries())/(m_pip_p->GetBinWidth(1)));
//m_pip_p->Draw();


m_pip_p_corr->Scale(1./(m_pip_p_corr->GetEntries())/(m_pip_p_corr->GetBinWidth(1)));
m_pip_p_corr->SetLineColor(kRed);

m_pip_p_corr->Divide(m_pip_p);
m_pip_p_corr->SetMinimum(0.);
m_pip_p_corr->SetMaximum(1.5);
m_pip_p_corr->Draw();

c->cd(2);

m_pip_pim->Scale(1./(m_pip_pim->GetEntries())/(m_pip_pim->GetBinWidth(1)));



m_pip_pim_corr->Scale(1./(m_pip_pim_corr->GetEntries())/(m_pip_pim_corr->GetBinWidth(1)));
m_pip_pim_corr->SetLineColor(kRed);

m_pip_pim_corr->Divide(m_pip_pim);
m_pip_pim_corr->SetMinimum(0.);
m_pip_pim_corr->SetMaximum(1.5);
m_pip_pim_corr->Draw();


c->cd(3);
m_pim_p->Scale(1./(m_pim_p->GetEntries())/(m_pim_p->GetBinWidth(1)));
//m_pim_p->Draw();


m_pim_p_corr->Scale(1./(m_pim_p_corr->GetEntries())/(m_pim_p_corr->GetBinWidth(1)));
m_pim_p_corr->SetLineColor(kRed);

m_pim_p_corr->Divide(m_pim_p);
m_pim_p_corr->SetMinimum(0.);
m_pim_p_corr->SetMaximum(1.5);
m_pim_p_corr->Draw();
...
void read_model(TFile *file_eff,Int_t i,Int_t qq2) {

file_eff->cd();

qqq.str("");
qqq << "q2_" << Q2_bin << "/w_" << W_bin[i] << "/model_pipP_1_";
gDirectory->GetObject(qqq.str().c_str(),m_pip_p_corr);
...
gDirectory->GetObject(qqq.str().c_str(),m_pip_pim_corr);

qqq.str("");
qqq << "q2_" << Q2_bin << "/w_" << W_bin[i] << "/model_pimP_1_";
gDirectory->GetObject(qqq.str().c_str(),m_pim_p_corr);
...
void read_data_all_w(TFile *file_eff,Int_t i,Int_t qq2) {

file_eff->cd();

qqq.str("");
qqq << "q2_" << Q2_bin << "/w_" << W_bin[i] << "/h_m_pip_pr_" << 425 + 50*qq2<< "_w_" << 13125 + 250*i;
...
qqq.str("");
qqq << "q2_" << Q2_bin << "/w_" << W_bin[i] << "/h_m_pip_pim_" << 425 + 50*qq2 <<"_w_" << 13125 + 250*i;
gDirectory->GetObject(qqq.str().c_str(),m_pip_pim);

qqq.str("");
qqq << "q2_" << Q2_bin << "/w_" << W_bin[i] << "/h_m_pim_pr_" << 425 + 50*qq2  <<"_w_" << 13125 + 250*i;
...
void read_data_cntr_w(TFile *file_eff,Int_t i, Int_t qq2) {
...
draw_1d_hist(1,m_pip_p,qqq.str(),"h1prj_inv_m_pip_p_1_","d#sigma/dM (#mubn/GeV)","m_{#pi^{+}p} (GeV)",1,"e1P","mass",i);
qqq.str("");

m_pip_p_bin_corr = h_bin_corr_mass(m_pip_p);

draw_1d_hist(1,m_pip_p_bin_corr,qqq.str(),"h1prj_inv_m_pip_p_1_gen","d#sigma/dM (#mubn/GeV)","m_{#pi^{+}p} (GeV)",2,"e1AP same","mass",i);
qqq.str("");

draw_1d_hist(1,m_pip_p_bin_corr,qqq.str(),"jhvj","d#sigma/dM (#mubn/GeV)","m_{#pi^{+}p} (GeV)",2,"e0AP same","mass",i);
qqq.str("");


//spline->Draw("same");

TF1 *f1 = new TF1("f1",f_fit,m_pip_p->GetBinLowEdge(1),m_pip_p->GetBinLowEdge(m_pip_p->GetNbinsX())+m_pip_p->GetBinWidth(1),0);
//f1->Draw("same");
...
draw_1d_hist(2,m_pip_pim,"","h1prj_inv_m_pip_pim_1_","d#sigma/dM (#mubn/GeV)","m_{#pi^{+}#pi^{-}} (GeV)",1,"e1P","mass",i);

m_pip_pim_bin_corr = h_bin_corr_mass(m_pip_pim);

draw_1d_hist(2,m_pip_pim_bin_corr,"","h1prj_inv_m_pip_pim_1_gen","d#sigma/dM (#mubn/GeV)","m_{#pi^{+}#pi^{-}} (GeV)",2,"e1AP same","mass",i);

draw_1d_hist(2,m_pip_pim_bin_corr,"","h1prj_inv_m_pip_pim_1_gen","d#sigma/dM (#mubn/GeV)","m_{#pi^{+}#pi^{-}} (GeV)",2,"e0AP same","mass",i);
...
draw_1d_hist(3,m_pim_p,"","h3prj_inv_m_pim_p_1_","d#sigma/dM (#mubn/GeV)","m_{#pi^{-}p} (GeV)",1,"e1P","mass",i);

m_pim_p_bin_corr = h_bin_corr_theta(m_pim_p);

draw_1d_hist(3,m_pim_p_bin_corr,"","h3prj_inv_m_pim_p_1_gen","d#sigma/dM (#mubn/GeV)","m_{#pi^{-}p} (GeV)",2,"e1AP same","mass",i);

draw_1d_hist(3,m_pim_p_bin_corr,"","h3prj_inv_m_pim_p_1_gen","d#sigma/dM (#mubn/GeV)","m_{#pi^{-}p} (GeV)",2,"e0AP same","mass",i);

//spline->Draw("same");
//f->Draw("same");

max = (theta_p->GetMaximum())/1.5;

theta_p->SetMaximum(max);


draw_1d_hist(4,theta_p,"","h1prj_th_P_","d#sigma/d(-cos#theta) (#mubn/rad)","#theta_{p'} in c.m. (deg)",1,"e1P","theta",i);

theta_p_bin_corr = h_bin_corr_theta(theta_p);

draw_1d_hist(4,theta_p_bin_corr,"","h1prj_th_P_gen","d#sigma/d(-cos#theta) (#mubn/rad)","#theta_{p'} in c.m. (deg)",2,"e1AP same","theta",i);

draw_1d_hist(4,theta_p_bin_corr,"","h1prj_th_P_gen","d#sigma/d(-cos#theta) (#mubn/rad)","#theta_{p'} in c.m. (deg)",2,"e0AP same","theta",i);
...
draw_1d_hist(5,theta_pim,"","h1prj_th_PIm_","d#sigma/d(-cos#theta) (#mubn/rad)","#theta_{#pi^{-}} in c.m. (deg)",1,"e1P","theta",i);

theta_pim_bin_corr = h_bin_corr_theta(theta_pim);

draw_1d_hist(5,theta_pim_bin_corr,"","h1prj_th_PIm_gen","d#sigma/d(-cos#theta) (#mubn/rad)","#theta_{#pi^{-}} in c.m. (deg)",2,"e1AP same","theta",i);
...
draw_1d_hist(6,theta_pip,"","h1prj_th_PIp_","d#sigma/d(-cos#theta) (#mubn/rad)","#theta_{#pi^{+}} in c.m. (deg)",1,"e1P","theta",i);

theta_pip_bin_corr = h_bin_corr_theta(theta_pip);

draw_1d_hist(6,theta_pip_bin_corr,"","h1prj_th_PIp_gen","d#sigma/d(-cos#theta) (#mubn/rad)","#theta_{#pi^{+}} in c.m. (deg)",2,"e1AP same","theta",i);
